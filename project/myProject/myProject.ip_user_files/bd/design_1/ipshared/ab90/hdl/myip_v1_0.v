...
// those which are assigned in an always block of myip_v1_0 shoud be changes to reg.
	...
	wire 	temp1_write_en;								
	wire 	[temp_depth_bits-1:0] temp1_write_address;		
	wire 	[width-1:0] temp1_write_data_in ;			
	wire	temp1_read_en;								
	wire	[temp_depth_bits-1:0] temp1_read_address;		
	wire	[width-1:0] temp1_read_data_out;
	wire	temp1_read1_en;								
	wire	[temp_depth_bits-1:0] temp1_read1_address;		
	wire	[width-1:0] temp1_read1_data_out;	

	wire 	temp2_write_en;								
	wire 	[temp_depth_bits-1:0] temp2_write_address ;		
	wire 	[width-1:0] temp2_write_data_in ;			
	wire	temp2_read_en;								
	wire	[temp_depth_bits-1:0] temp2_read_address;		
	wire	[width-1:0] temp2_read_data_out;	
	wire	temp2_read1_en;								
	...
	wire	sig_read_en;								
	wire	[sig_depth_bits-1:0] sig_read_address;		
	wire	[width-1:0] sig_read_data_out;	
	wire	sig_read1_en;								
	...
	wire    sig_en1, sig_en2;
	wire [width-1:0] sig_data1, sig_data2;
	wire [RES_depth_bits-1:0] sig_add1, sig_add2;
	wire sig_activ, sig_activ2;
   ...
   reg [B_depth_bits:0] nr_of_b2_reads = 0; //counter of b reads
   ...
   reg [10:0] nr_of_reads = 0;   // to do : change it as necessary
   reg [RES_depth_bits - 1:0] nr_of_writes = 0; // to do : change it as necessary
					...
					Read_Inputs :
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    B2_write_en <= 0;
                                    sig_write_address <= NUMBER_SIGMOID_INPUT - nr_of_sig_reads;
                                    sig_write_data_in <= S_AXIS_TDATA[7:0];
                                    nr_of_sig_reads   <= nr_of_sig_reads - 1;
                                end
                                else if(nr_of_reads == 3)
                                begin
                                    sig_write_en <= 0;
...
matrix_multiply #(.width(width), .A_depth_bits(A_depth_bits), .B_depth_bits(B_depth_bits), .RES_depth_bits(RES_depth_bits)) matrix_multiply_0 (
	...
	.RES_write_en     (sig_en1         ),
	.RES_write_address(sig_add1        ),
	.RES_write_data_in(sig_data1       )
);
...
matrix_multiply #(.width(width), .A_depth_bits(A_depth_bits), .B_depth_bits(B_depth_bits), .RES_depth_bits(RES_depth_bits)) matrix_multiply_1 (
	...
	.A_read_en        (A_read1_en      ),
	.A_read_address   (A_read1_address ),
	.A_read_data_out  (A_read1_data_out),
	
	.B_read_en        (B2_read_en      ),
	.B_read_address   (B2_read_address ),
	.B_read_data_out  (B2_read_data_out),
	
	.RES_write_en     (sig_en2         ),
	.RES_write_address(sig_add2        ),
	.RES_write_data_in(sig_data2       )
);

sigmoid_function #(.width(width), .A_depth_bits(temp_depth_bits), .SIG_depth_bits(sig_depth_bits)) sigmoid_function_1 (
	.clk               (ACLK               ),
	.Start             (sig_en1            ),
	.Done              (sig_activ          ),
...
sigmoid_function #(.width(width), .A_depth_bits(temp_depth_bits), .SIG_depth_bits(sig_depth_bits)) sigmoid_function_2 (
	...
	.SIG_read_en       (sig_read1_en        ),
	.SIG_read_address  (sig_read1_address   ),
	.SIG_read_data_out (sig_read1_data_out  )
);

output_layer_mat_mult#(.width(width), .A_depth_bits(temp_depth_bits), .RES_depth_bits(RES_depth_bits)) output_layer1(
	.clk               (ACLK               ),
	.Start             (sig_activ          ),
	.Done              (Done               ),
	.temp_address      (temp1_write_address),

	.A1_read_en        (temp1_read_en      ),
	.A1_read_address   (temp1_read_address ),
	.A1_read_data_out  (temp1_read_data_out),

	.A2_read_en        (temp2_read_en      ),
	.A2_read_address   (temp2_read_address ),
	.A2_read_data_out  (temp2_read_data_out),
